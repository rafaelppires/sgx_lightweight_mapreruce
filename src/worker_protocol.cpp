...
void WorkerProtocol::initialsub() {
    ...
    if( encrypt_ ) {
        ...
        key[0] = 'a'; key[15] = '5';
        iv[0] = 'x'; iv[15]= '?';
        encrypt_aes( AES128, (const uint8_t*)header.c_str(), (uint8_t*)buff, sz, key, iv );
...
void WorkerProtocol::actual_output( char *buff, size_t len,
    ...
    if( encrypt_ ) {
        ...
        encrypt_aes( AES128, (const uint8_t*)header.c_str(), (uint8_t*)buff,      hdsz,key,iv );
        encrypt_aes( AES128, (const uint8_t*)payload.c_str(), (uint8_t*)buff+hfsz, plsz, key, iv );
